...
active_notes = {}   # key -> {start_time, finger}
midi_events = []
finger_log = []
    ...
    if results.multi_hand_landmarks:
        for hand in results.multi_hand_landmarks:
            for idx in FINGERTIPS:
                ...
                x = int(lm.x * W)
                y = int(lm.y * H)
                if key_ylim[0] - 50 < y < key_ylim[1] + 30:
                    fingers.append({
                        "id": idx,
                        "x": x,
                        "y": y,
...
def match_keys_and_fingers(keys, fingers):
    matches = []
    used = set()
    ...
    for k in keys:
        ...
        if best is not None:
            used.add(best)
            matches.append((k, fingers[best]))

    return matches

def build_white_key_midi_map(start_midi, n_keys):
    midi_map = [start_midi]
    i = 0
    while len(midi_map) < n_keys:
        midi_map.append(midi_map[-1] + WHITE_KEY_INTERVALS[i % 7])
        i += 1
    return midi_map
    ...
    if ret:
        ...
        for k in range(len(key_boundaries) - 1):
            ...
            if mode == Mode.CLASSIFICATION:
                ...
                press_count[k] = press_count.get(k, 0)
                ...
                if press_count[k] == ONSET_GRACE_FRAMES:
                    pressed_onsets.append(k)
                
                if not pressed and prev_press_states[k]:
                    note = active_notes.pop(k, None)
                    if note:
                        duration = t - note["start"]
                        if duration >= MIN_DURATION:
                            midi_note = WHITE_MIDI_MAP[k]
                            midi_events.append((note["start"], "on", midi_note))
                            midi_events.append((t, "off", midi_note))
                            finger_log.append({
                                "start_time": note["start"],
                                "end_time": t,
                                "key_index": k,
                                "midi_note": midi_note,
                                "finger_id": note["finger"]["id"],
                                "x": note["finger"]["x"],
                                "y": note["finger"]["y"],
                                "z": note["finger"]["z"],
                            })
            ...
            if pressed_onsets:
                fingers = get_fingertips(frame)
                if not fingers:
                    # 指が取れなくても鍵盤押下は仮登録
                    for k in pressed_onsets:
                       active_notes[k] = {
                            "start": t,
                            "finger": {
                                "id": -1, "x": -1, "y": -1, "z": 0
                    ...
                    matches = match_keys_and_fingers(pressed_onsets, fingers)
                    for k, f in matches:
                        active_notes[k] = {"start": t, "finger": f}
...
with open("finger_log.csv", "w", newline="") as f:
    ...
    writer.writeheader()
    writer.writerows(finger_log)

with open("midi_events.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["time_sec", "type", "midi_note"])
    for e in sorted(midi_events):
        writer.writerow(e)
